...
use RTsynth_module, only : C,HP,KB
...
double precision, intent(out)::ENERGIES(1:NLEV), WEIGHTS(1:NLEV)
double precision, intent(out)::A_COEFFS(1:NLEV,1:NLEV), B_COEFFS(1:NLEV,1:NLEV)
double precision, intent(out)::FREQUENCIES(1:NLEV,1:NLEV), TEMPERATURES(1:7,1:NTEMP)
double precision, intent(out)::H_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::HP_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::EL_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::HE_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::H2_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::PH2_COL(1:NLEV,1:NLEV,1:NTEMP)
double precision, intent(out)::OH2_COL(1:NLEV,1:NLEV,1:NTEMP)

INTEGER::NLIN,NPART,NCOL
INTEGER::I,J,K,L,M,N,P
double precision:: ENERGY,WEIGHT,EINSTEINA,FREQUENCY
double precision::COEFF(1:NTEMP)
