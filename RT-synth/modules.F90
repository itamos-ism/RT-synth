...
type coolant_node
  ...
  double precision, pointer :: Vunits(:,:) !unit conversion to K
end type coolant_node
type(coolant_node), allocatable::coolant(:)
integer::cur_nlev,cur_ntemp

character(len=10)::chemsuf
character(len=100)::prefix,directory,filein
character(len=50)::filevel,velfile
character(len=10),allocatable::species(:)
character(len=2) :: los_direction  ! Possible values: '+x', '-x', '+y', '-y', '+z', '-z'

integer::ctot,jr,lr
type temp_coolant_node
  double precision,pointer :: pop(:)
end type temp_coolant_node
type(temp_coolant_node), allocatable :: temp_coolant(:)
#ifdef VELOCITY
integer :: velmin, velmax, velstep, vel
real :: ds_freq
#endif

integer::etype,itot,nspec
character(len=10)::id
double precision::uv,dummy

double precision :: Ntot
double precision, allocatable :: Nabn(:)
#ifdef CRATTENUATION
double precision :: Ncr
#endif

#ifdef VELOCITY
double precision,allocatable :: temp_tTr(:,:,:)
#else
double precision :: temp_tTr
#endif
...
type pdr_node
   ...
   double precision :: tpop(1:2)
   double precision :: zetalocal
#ifdef CRATTENUATION
   double precision :: Ncr
#endif
#ifdef VELOCITY
   double precision :: W
   double precision,pointer :: ttau(:)
   double precision,pointer :: tTr(:)
#ifdef HI21CM
   double precision :: W_HI
   double precision,pointer :: tau_HI(:), T_HI(:)
#endif
#else
   double precision :: ttau
   double precision :: tTr
#endif
   double precision :: tTex
   double precision :: tBnu
#ifdef DUST
#ifdef VELOCITY
   double precision, pointer :: tSource(:)
#else
...
double precision :: redshift,Tcmb
#ifdef DUST
double precision :: alpha_dust, Bnu_dust
double precision :: d2g
#endif

double precision :: dtau, Tr_incr1, Tr_incr2
double precision :: vturb

end module RTsynth_module
